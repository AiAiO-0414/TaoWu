<template>
  <div class="cart-container">
      <van-button v-if="$store.state.goodsCartData.length > 0" block type="info" plain @click="$router.push('/addressedit')">添加地址</van-button>
    <div class="cartlist" v-for="(item,index) in $store.state.goodsCartData" :key="item.id">
      ...
      <van-stepper :value="item.number" min="0" :name="index" @change="AddNumber" />
    </div>
    <van-submit-bar
    ...
    >
      ...
      >共选中 {{$store.getters.getTotalNumber }} 件</van-checkbox>
...
export default {
  data() {
    return {
      emptyImg,
      goodsCartList: [],
      ids: this.$store.state.goodsCartData,
    };
  },
  created() {
  ...
  methods: {
    ...
    //   let { message } = fetchGoodsCart(this.ids);
